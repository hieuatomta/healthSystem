...
@Component({
  ...
  selector: 'ngx-users',
  styleUrls: ['./products-selling.component.scss'],
...
export class ProductsSellingComponent implements OnInit {
  ...
  exportAsCSV() {
    ...
    //   }
    // }
    this.excelService.exportAsExcelFile(this.rows, 'top_san_pham_data');
  }
  ...
  columns = [
    ...
    {name: 'common.table.item_product_name', prop: 'nameProducts', flexGrow: 1.5},
    {name: 'common.table.item_size_name', prop: 'nameSize', flexGrow: 1.5},
    {name: 'common.table.item_product_total', prop: 'totalOrder', flexGrow: 1},
  ];
  ...
  search(pageToLoad: number) {
    ...
    this.statisticalService.getSanPhamBanChay(this.dateDate
    ).subscribe(
      (res) => {
        console.log(res);

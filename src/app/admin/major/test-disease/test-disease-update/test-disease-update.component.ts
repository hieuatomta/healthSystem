...
@Component({
  ...
  styleUrls: ['./test-disease-update.component.scss'],
  templateUrl: './test-disease-update.component.html',
})
export class TestDiseaseUpdateComponent implements OnInit {
  lstRole = [];
  listStatus = [
  ...
  constructor(
    ...
    public ref: NbDialogRef<TestDiseaseUpdateComponent>,
  ...
  submit() {
    ...
    if (this.inputSize.valid) {
      this.loading = true;
      if (this.data == null) {
        this.testDiseaseService.insert(this.inputSize.value).subscribe(
        ...
        this.testDiseaseService.update(this.inputSize.value).subscribe(
          (value) => this.ref.close(value),
          (error) => {
            this.toastr.danger(error.error.message, this.translate.instant('common.title_notification'));
            this.loading = false;
